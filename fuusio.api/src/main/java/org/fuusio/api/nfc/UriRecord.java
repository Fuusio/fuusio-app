...
public class UriRecord extends ParsedNdefRecord {
    ...
    private static UriRecord parseWellKnown(final NdefRecord pRecord) {
         ...
         */
        final String prefix = URI_PREFIX_MAP.get(payload[0]);
        final byte[] fullUri = ByteToolkit.concatenate(prefix.getBytes(CHARSET_UTF_8),
                Arrays.copyOfRange(payload, 1, payload.length));
        final Uri uri = Uri.parse(new String(fullUri, CHARSET_UTF_8));
        return new UriRecord(uri);
    }
